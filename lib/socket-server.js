...
// Load environment from .env.local first (Next-style), then fallback to .env
...
async function start() {
  ...
  socket.on('chat:message', async (payload) => {
    ...
    const msg = { ...payload, id: new ObjectId().toString(), createdAt: now, reservationId: payload.reservationId || null }
        ...
        try {
          ...
          if (recipientId) {
            const notif = {
              ...
              metadata: { listingId: toStringId(listing._id) || listing.id, reservationId: payload.reservationId || undefined }
            }
